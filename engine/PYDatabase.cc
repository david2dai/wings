...
// David Add for Feedback
#define DB_BACKUP_TIMEOUT_FEEDBACK   (60)
...
Database::Database (void)
    : m_db (NULL)
    , m_timeout_id (0)
    , m_timeout_id_feedback (0)
    , m_timer (g_timer_new ())
    , m_timer_feedback (g_timer_new ())
{
    open ();
}
...
Database::~Database (void)
{
    ...
    g_timer_destroy (m_timer_feedback);
    if (m_timeout_id != 0) {
        saveUserDB ();
        g_source_remove (m_timeout_id);
    }

    if (m_timeout_id_feedback != 0) {
        saveFeedbackDB ();
        g_source_remove (m_timeout_id_feedback);
    } 
...
Database::open (void)
{
    do {
        ...
        qryFeedback (1, recIds, freqs, REC_TYPE_WORD);
       
        log.logInfo ("[%s][%s] recWords recWordsSize [%d] freq size[%d]!\n",
                __FILE__,
                __FUNCTION__,
                recIds.size (),
                freqs.size ());
        ...
        for (int i=0;i<recIds.size ();++i) {
            ...
            log.logInfo ("[%s][%s] recWords id [%d] freq[%d]!\n",
                __FILE__,
                __FUNCTION__,
                recIds[i],
                freqs[i]);
        ...
        int freq1 = qryFeedback (2, 1, REC_TYPE_SENTENCE);
        int freq2 = qryFeedback (2, 2, REC_TYPE_SENTENCE);
        int freq3 = qryFeedback (1, 2, REC_TYPE_SENTENCE);
        int freq4 = qryFeedback (1, 1, REC_TYPE_SENTENCE);
        
        log.logInfo ("[%s][%s] 1[%d] 2[%d] 3[%d] 4[%d] !\n",
                __FILE__,
                __FUNCTION__,
                freq1,
                freq2,
        ...
        addFeedback (2, 1, REC_TYPE_SENTENCE);
        addFeedback (2, 2, REC_TYPE_SENTENCE);
        addFeedback (1, 2, REC_TYPE_SENTENCE);
        addFeedback (1, 1, REC_TYPE_SENTENCE);

        freq1 = qryFeedback (2, 1, REC_TYPE_SENTENCE);
        freq2 = qryFeedback (2, 2, REC_TYPE_SENTENCE);
        freq3 = qryFeedback (1, 2, REC_TYPE_SENTENCE);
        freq4 = qryFeedback (1, 1, REC_TYPE_SENTENCE);
...
// For feedback
gboolean 
Database::loadFeedbackDB (void)
{
    sqlite3 *feedbackdb = NULL;
    do {
        /* Attach user database */
        m_sql.printf ("ATTACH DATABASE \":memory:\" AS feedbackdb;");
        if (!executeSQL (m_sql)) {
        ...
        m_buffer << G_DIR_SEPARATOR_S << "ibus"
                 << G_DIR_SEPARATOR_S << "pinyin";
        g_mkdir_with_parents (m_buffer, 0750);
        m_buffer << G_DIR_SEPARATOR_S << "feedback.db";

        gint flags = SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE;
        if (sqlite3_open_v2 (m_buffer, &feedbackdb, flags, NULL) != SQLITE_OK &&
            sqlite3_open_v2 (":memory:", &feedbackdb, flags, NULL) != SQLITE_OK) {
        ...
        // need create two tables, one for words and one for sentences.
        m_sql   << "CREATE TABLE IF NOT EXISTS words "
                << "(id INTEGER PRIMARY KEY AUTOINCREMENT,"
                << "orig_word_id INTEGER KEY, rec_word_id INTEGER KEY, freq INTEGER);\n";
        /*
        m_sql << "INSERT OR IGNORE INTO words VALUES " << "(NULL,1111, 2222, 5);\n";
        m_sql << "INSERT OR IGNORE INTO words VALUES " << "(NULL,1111, 2223, 6);\n";
        m_sql << "INSERT OR IGNORE INTO words VALUES " << "(NULL,1112, 2223, 7);\n";*/
        m_sql   << "CREATE TABLE IF NOT EXISTS sentences "
                << "(id INTEGER PRIMARY KEY AUTOINCREMENT," 
                << "orig_word_id INTEGER KEY, rec_sentence_id INTEGER KEY, freq INTEGER);\n";
        //m_sql << "INSERT OR IGNORE INTO sentences VALUES " << "(NULL,111111, 3000000, 5);\n";
        m_sql << "COMMIT;";

        if (!executeSQL (m_sql, feedbackdb)) {

            break;
        }

        sqlite3_backup *backup = sqlite3_backup_init (m_db, "feedbackdb", feedbackdb, "main");
...
Database::saveFeedbackDB (void)
{
    ...
    do {
        ...
        gint flags = SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE;
        if (sqlite3_open_v2 (tmpfile, &feedbackdb, flags, NULL) != SQLITE_OK)
            break;

        sqlite3_backup *backup = sqlite3_backup_init (feedbackdb, "main", m_db, "feedbackdb");
...
gboolean
Database::timeoutCallback_feedback (gpointer data)
{
    Database *self = static_cast<Database*> (data);

    /* Get elapsed time since last modification of database. */
    guint elapsed = (guint)g_timer_elapsed (self->m_timer_feedback, NULL);

   
    if (elapsed >= DB_BACKUP_TIMEOUT_FEEDBACK &&
        self->saveFeedbackDB () ) {
        self->m_timeout_id_feedback  = 0;
        return FALSE;
    }
...
void
Database::modified_feedback (void)
{
    /* Restart the timer */
    g_timer_start (m_timer_feedback);

    if (m_timeout_id_feedback != 0)
        return;

    m_timeout_id_feedback  = g_timeout_add_seconds (DB_BACKUP_TIMEOUT_FEEDBACK,
                                                    Database::timeoutCallback_feedback ,
...
// my
void
Database::addFeedback (long origWordId, long recId, int type)
{
   
    int freq = 0;
    freq = qryFeedback (origWordId, recId, type); // return freq
    
    m_sql = "BEGIN TRANSACTION;\n";

    if (0==freq) {

        if (REC_TYPE_WORD==type) {
            m_sql << "INSERT OR  IGNORE INTO feedbackdb.words VALUES ";
            m_sql.appendPrintf(" (NULL,%ld,%ld,1);\n", origWordId,recId);
        } else {
            m_sql << "INSERT INTO feedbackdb.sentences VALUES ";
            m_sql.appendPrintf(" (NULL,%ld,%ld,1);\n", origWordId,recId);
        }
        ...
        if (REC_TYPE_WORD==type) {
            m_sql << "UPDATE feedbackdb.words SET freq= " << freq +1;
            m_sql.appendPrintf(" WHERE orig_word_id=%ld AND rec_word_id=%ld;\n", origWordId, recId);
        } else {
            m_sql << "UPDATE feedbackdb.sentences SET freq= " << freq +1;
            m_sql.appendPrintf(" WHERE orig_word_id=%ld AND rec_sentence_id=%ld;\n ", origWordId, recId);
        }
...
// qry all the words/sentences freq of a original word
void 
Database::qryFeedback (long origWordId, std::map<long,int> & idFreqs, int type) 
{
    m_sql.clear ();
    long recId = -1;
    int freq = -1;
    ...
    if (type == REC_TYPE_WORD) {
        
        m_sql << "SELECT * FROM feedbackdb.words  WHERE  orig_word_id="; // << " and orig_word_id=" << origWordId 
        m_sql.appendPrintf("%ld", origWordId);
        ...
        while (stmt->step ()) {
            recId  = stmt->columnInt (2);
            freq = stmt->columnInt (3);
        ...
        m_sql << "SELECT * FROM feedbackdb.sentences WHERE  orig_word_id="; // << " and orig_word_id=" << origWordId 
        m_sql.appendPrintf("%ld", origWordId);
...
// qry the orignal - word/sentence freq
// this will be used in addFeedback function
int 
Database::qryFeedback (long origWordId, long recId, int type) // return freq
{
    m_sql.clear ();
    int freq = 0;
    ...
    if (type == REC_TYPE_WORD) {
        ...
        m_sql << "SELECT freq FROM feedbackdb.words  WHERE "; // << " and orig_word_id=" << origWordId 
        m_sql.appendPrintf(" orig_word_id=%ld AND rec_word_id=%ld ", origWordId, recId);
        ...
        m_sql << "SELECT freq FROM feedbackdb.sentences WHERE "; // << " and orig_word_id=" << origWordId 
        m_sql.appendPrintf(" orig_word_id=%ld AND rec_sentence_id=%ld ", origWordId, recId);
