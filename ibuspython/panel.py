...
class PanelBase(Object):
    ...
    # David
    # DBus interface method will call the method 
    # this method need be over write by the subclass 
    ...
    # David
    # For rec words
    def page_up_rec_words(self):
        self.__proxy.PageUpRecWords ()

    def page_down_rec_words(self):
        self.__proxy.PageDownRecWords ()

    def cursor_up_rec_words(self):
        self.__proxy.CursorUpRecWords()

    def cursor_down_rec_words(self):
        self.__proxy.CursorDownRecWords()

    def candidate_clicked_rec_words(self, index, button, state):
        self.__proxy.CandidateClickedRecWords(index, button, state)

    # For rec senteneces 
    def page_up_rec_sentences(self):
        self.__proxy.PageUpRecSentences()

    def page_down_rec_sentences(self):
        self.__proxy.PageDownRecSentences()

    def cursor_up_rec_sentences(self):
        self.__proxy.CursorUpRecSentences()

    def cursor_down_rec_sentences(self):
        self.__proxy.CursorDownRecSentences()

    def candidate_clicked_rec_sentences(self, index, button, state):
        self.__proxy.CandidateClickedRecSentences(index, button, state)
    ...
    # David
    def focused_rec_word(self, sel_rec_word):
        #outfile = open("/home/david/David/log.txt","a")
        #outfile.write("ibus/panel.py:before["+str(sel_rec_word)+"] ["+str(sel_rec_word.text)+"]\n")
        #outfile.close()
        sel_rec_word = serialize_object(sel_rec_word)
        #outfile = open("/home/david/David/log.txt","a")
        #outfile.write("ibus/panel.py:after\n")
        #outfile.close()
        self.__proxy.FocusedRecWord(sel_rec_word)

    def focused_rec_sentence(self, sel_rec_sentence):
        sel_rec_sentence = serializable.serialize_object(sel_rec_sentence)
        self.__proxy.FocusedRecSentence(sel_rec_sentence)
...
class PanelProxy(interface.IPanel):
    ...
    # David
    def UpdateLookupTableRecWords(self, lookup_table, visible):
        lookup_table = deserialize_object(lookup_table)
        self.__panel.update_lookup_table_rec_words(lookup_table, visible)

    def ShowLookupTableRecWords(self):
        self.__panel.show_lookup_table_rec_words()

    def HideLookupTableRecWords(self):
        self.__panel.hide_lookup_table_rec_words()

    # David
    def UpdateLookupTableRecSentences(self, lookup_table, visible):
        lookup_table = deserialize_object(lookup_table)
        self.__panel.update_lookup_table_rec_sentences(lookup_table, visible)

    def ShowLookupTableRecSentences(self):
        self.__panel.show_lookup_table_rec_sentences()

    def HideLookupTableRecSentences(self):
        self.__panel.hide_lookup_table_rec_sentences()
    ...
    # David
    # DBus method
    # add for update the indicator for the  focused area
    # 3 focused area: original words area, rec words area, rec sentence area
    def UpdateFocusedAreaIndicator(self, indicator):
        self.__panel.update_focused_area_indicator(indicator)
