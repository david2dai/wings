...
enum {
    ...
    REGISTER_PROPERTIES,
    UPDATE_PROPERTY,
    UPDATE_FOCUSED_AREA_INDICATOR, // David
    LAST_SIGNAL,
};
...
bus_engine_proxy_class_init (BusEngineProxyClass *class)
{
    ...
    engine_signals[UPDATE_LOOKUP_TABLE_REC_WORDS] =
    ...
    engine_signals[UPDATE_LOOKUP_TABLE_REC_SENTENCES] =
    ...
    engine_signals[SHOW_LOOKUP_TABLE_REC_WORDS] =
    ...
    engine_signals[SHOW_LOOKUP_TABLE_REC_SENTENCES] =
    ...
    engine_signals[HIDE_LOOKUP_TABLE_REC_WORDS] =
    ...
    engine_signals[HIDE_LOOKUP_TABLE_REC_SENTENCES] =
    ...
    // David
    engine_signals[UPDATE_FOCUSED_AREA_INDICATOR] =
                           ...
                           GVariant    *parameters)
{
    ...
    static const struct {
        ...
        { "ShowLookupTableRecWords",        SHOW_LOOKUP_TABLE_REC_WORDS },
        { "HideLookupTableRecWords",        HIDE_LOOKUP_TABLE_REC_WORDS },
        { "ShowLookupTableRecSentences",        SHOW_LOOKUP_TABLE_REC_SENTENCES },
        { "HideLookupTableRecSentences",        HIDE_LOOKUP_TABLE_REC_SENTENCES },
    ...
    // David
    if (g_strcmp0 (signal_name, "UpdateFocusedAreaIndicator") == 0) {
        guint indicator = 0;

        g_variant_get (parameters, "(u)", &indicator);

        /*
        FILE *outfile; 
        outfile = fopen("/home/david/David/tablog","at");
        char buffer[256] = {0};
        sprintf(buffer,"engineproxy.c: Get DBUS signal UpdateFocusedAreaIndicator[%d]\n",indicator);
    ...
    if (g_strcmp0 (signal_name, "UpdateLookupTableRecWords") == 0) {
        GVariant *arg0 = NULL;
        gboolean visible = FALSE;
        ...
        g_signal_emit (engine, engine_signals[UPDATE_LOOKUP_TABLE_REC_WORDS], 0, table, visible);
    ...
    if (g_strcmp0 (signal_name, "UpdateLookupTableRecSentences") == 0) {
        ...
        g_signal_emit (engine, engine_signals[UPDATE_LOOKUP_TABLE_REC_SENTENCES], 0, table, visible);
...
DEFINE_FUNCTION (PageUpRecWords, page_up_rec_words)
DEFINE_FUNCTION (PageDownRecWords, page_down_rec_words)
DEFINE_FUNCTION (CursorUpRecWords, cursor_up_rec_words)
DEFINE_FUNCTION (CursorDownRecWords, cursor_down_rec_words)

DEFINE_FUNCTION (PageUpRecSentences, page_up_rec_sentences)
DEFINE_FUNCTION (PageDownRecSentences, page_down_rec_sentences)
DEFINE_FUNCTION (CursorUpRecSentences, cursor_up_rec_sentences)
DEFINE_FUNCTION (CursorDownRecSentences, cursor_down_rec_sentences)
...
// David
void
bus_engine_proxy_focused_rec_word (BusEngineProxy *engine,
                                   IBusText       *sel_rec_word)
{
    g_assert (BUS_IS_ENGINE_PROXY (engine));

    GVariant *variant = ibus_serializable_serialize ((IBusSerializable *) sel_rec_word);
    ...
    g_dbus_proxy_call ((GDBusProxy *)engine,
                       ...
                       NULL);
    // David 
    // I think sel_rec_word can be deleted here now

}

void
bus_engine_proxy_focused_rec_sentence (BusEngineProxy *engine,
                                       IBusText       *sel_rec_sentence)
{
    g_assert (BUS_IS_ENGINE_PROXY (engine));

    GVariant *variant = ibus_serializable_serialize ((IBusSerializable *) sel_rec_sentence);
    g_dbus_proxy_call ((GDBusProxy *)engine,
                       "FocusedRecSentence",
                       g_variant_new ("(v)", variant),
                       G_DBUS_CALL_FLAGS_NONE,
...
// candidate clicked
void
bus_engine_proxy_candidate_clicked_rec_words (BusEngineProxy *engine,
                                    guint           index,
                                    guint           button,
                                    guint           state)
{
...
// candidate clicked
void
bus_engine_proxy_candidate_clicked_rec_sentences (BusEngineProxy *engine,
                                    guint           index,
                                    guint           button,
                                    guint           state)
{
