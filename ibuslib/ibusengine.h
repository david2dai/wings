...
struct _IBusEngineClass {
    ...
    void        (* candidate_clicked)
                                    (IBusEngine     *engine,
                                     ...
                                     guint           state);
    // David
    void        (* focused_rec_word)
                                    (IBusEngine     *engine,
                                     IBusText       *sel_rec_word);
    void        (* focused_rec_sentence)
                                    (IBusEngine     *engine,
                                     IBusText       *sel_rec_sentence);
    
    void        (* page_up_rec_words)         (IBusEngine     *engine);
    void        (* page_down_rec_words)       (IBusEngine     *engine);
    void        (* cursor_up_rec_words)       (IBusEngine     *engine);
    void        (* cursor_down_rec_words)     (IBusEngine     *engine);

    void        (* candidate_clicked_rec_words)
                                    (IBusEngine     *engine,
                                     guint           index,
                                     guint           button,
                                     guint           state);
       
    void        (* page_up_rec_sentences)         (IBusEngine     *engine);
    void        (* page_down_rec_sentences)       (IBusEngine     *engine);
    void        (* cursor_up_rec_sentences)       (IBusEngine     *engine);
    void        (* cursor_down_rec_sentences)     (IBusEngine     *engine);

    void        (* candidate_clicked_rec_sentences)
                                    (IBusEngine     *engine,
                                     guint           index,
                                     guint           button,
                                     guint           state);
...
/**
 * ibus_engine_update_focused_area_indicator:
 * @engine: An IBusEngine.
 * @indicator: panel focused area indicator.
 *
 * Updte panel focused area indicator.
 *
 */
...
/**
 * ibus_engine_update_lookup_table_rec_words:
 * @engine: An IBusEngine.
...
void         ibus_engine_update_lookup_table_rec_words 
                                                       (IBusEngine         *engine,
                                                        IBusLookupTable    *lookup_table,
                                                        gboolean            visible);

void         ibus_engine_update_lookup_table_rec_sentences
                                                       (IBusEngine         *engine,
                                                        IBusLookupTable    *lookup_table,
                                                        gboolean            visible);

/**
 * ibus_engine_update_lookup_table_fast_rec_words:
 * @engine: An IBusEngine.
 * @lookup_table: An lookup_table.
 * @visible: Whether the lookup_table is visible.
 *
 ...
 * then it calls ibus_engine_update_lookup_table_rec_words().
 *
 * (Note: The table object will be released, if it is floating.
 *  If caller want to keep the object, caller should make the object
 *  sink by g_object_ref_sink.)
 */
void         ibus_engine_update_lookup_table_fast_rec_words
                                        (IBusEngine         *engine,
                                         IBusLookupTable    *lookup_table,
                                         gboolean            visible);

void         ibus_engine_update_lookup_table_fast_rec_sentences
                                        (IBusEngine         *engine,
                                         IBusLookupTable    *lookup_table,
                                         gboolean            visible);
...
/**
 * ibus_engine_show_lookup_table_rec_words:
 * @engine: An IBusEngine.
 *
 * Show the lookup table.
 */
void         ibus_engine_show_lookup_table_rec_words
                                        (IBusEngine         *engine);

void         ibus_engine_show_lookup_table_rec_sentences
                                        (IBusEngine         *engine);


/**
 * ibus_engine_hide_lookup_table_rec_words:
 * @engine: An IBusEngine.
 *
 * Hide the lookup table.
 */
void         ibus_engine_hide_lookup_table_rec_words
                                        (IBusEngine         *engine);

void         ibus_engine_hide_lookup_table_rec_sentences
                                        (IBusEngine         *engine);
