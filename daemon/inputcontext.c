...
struct _BusInputContext {
    ...
    /* lookup table rec words */
    IBusLookupTable *lookup_table_rec_words;
    gboolean lookup_table_visible_rec_words;

    /* lookup table rec words */
    IBusLookupTable *lookup_table_rec_sentences;
    gboolean lookup_table_visible_rec_sentences;
...
enum {
    ...
    SHOW_LOOKUP_TABLE,
    HIDE_LOOKUP_TABLE,
    UPDATE_LOOKUP_TABLE_REC_WORDS,
    UPDATE_LOOKUP_TABLE_REC_SENTENCES,
    SHOW_LOOKUP_TABLE_REC_WORDS,
    HIDE_LOOKUP_TABLE_REC_WORDS,
    SHOW_LOOKUP_TABLE_REC_SENTENCES,
    HIDE_LOOKUP_TABLE_REC_SENTENCES,
...
static void     bus_input_context_hide_preedit_text
                                                (BusInputContext        *context);
static void     bus_input_context_update_focused_area_indicator 
                                                (BusInputContext *context,
                                                 guint           indicator);
...
// David
static void     bus_input_context_show_lookup_table_rec_words
                                                (BusInputContext        *context);
static void     bus_input_context_hide_lookup_table_rec_words
                                                (BusInputContext        *context);

static void     bus_input_context_show_lookup_table_rec_sentences
                                                (BusInputContext        *context);
static void     bus_input_context_hide_lookup_table_rec_sentences
                                                (BusInputContext        *context);
...
static const gchar introspection_xml[] =
    ...
    "    <signal name='ShowLookupTableRecWords'/>"
    "    <signal name='HideLookupTableRecWords'/>"
...
bus_input_context_class_init (BusInputContextClass *class)
{
    ...
    // David
    context_signals[UPDATE_FOCUSED_AREA_INDICATOR] =
        g_signal_new (I_("update-focused-area-indicator"),
            G_TYPE_FROM_CLASS (class),
            G_SIGNAL_RUN_LAST,
            0,
            NULL, NULL,
            bus_marshal_VOID__UINT,
            G_TYPE_NONE,
            1,
            G_TYPE_UINT);
    ...
    context_signals[UPDATE_LOOKUP_TABLE_REC_WORDS] =
        g_signal_new (I_("update-lookup-table-rec-words"),
            G_TYPE_FROM_CLASS (class),
            G_SIGNAL_RUN_LAST,
            0,
            NULL, NULL,
    ...
    context_signals[UPDATE_LOOKUP_TABLE_REC_SENTENCES] =
        g_signal_new (I_("update-lookup-table-rec-sentences"),
            G_TYPE_FROM_CLASS (class),
            G_SIGNAL_RUN_LAST,
            0,
            NULL, NULL,
    ...
    // David
    context_signals[SHOW_LOOKUP_TABLE_REC_WORDS] =
        g_signal_new (I_("show-lookup-table-rec-words"),
            G_TYPE_FROM_CLASS (class),
            G_SIGNAL_RUN_LAST,
            0,
            NULL, NULL,
    ...
    context_signals[HIDE_LOOKUP_TABLE_REC_WORDS] =
        g_signal_new (I_("hide-lookup-table-rec-words"),
            G_TYPE_FROM_CLASS (class),
            G_SIGNAL_RUN_LAST,
            0,
            NULL, NULL,
    ...
    context_signals[SHOW_LOOKUP_TABLE_REC_SENTENCES] =
        g_signal_new (I_("show-lookup-table-rec-sentences"),
            G_TYPE_FROM_CLASS (class),
            G_SIGNAL_RUN_LAST,
            0,
            NULL, NULL,
    ...
    context_signals[HIDE_LOOKUP_TABLE_REC_SENTENCES] =
        g_signal_new (I_("hide-lookup-table-rec-sentences"),
            G_TYPE_FROM_CLASS (class),
            G_SIGNAL_RUN_LAST,
            0,
            NULL, NULL,
        ...
        if (context->engine && context->enabled) {
            ...
            if (context->lookup_table_visible && (context->capabilities & IBUS_CAP_LOOKUP_TABLE) == 0) {
                ...
                g_signal_emit (context,
                               context_signals[UPDATE_LOOKUP_TABLE_REC_WORDS],
                               0,
                               context->lookup_table_rec_words,
                               context->lookup_table_visible_rec_words);

                // David
                g_signal_emit (context,
                               context_signals[UPDATE_LOOKUP_TABLE_REC_SENTENCES],
                               0,
                               context->lookup_table_rec_sentences,
                               context->lookup_table_visible_rec_sentences);
...
// David
void
bus_input_context_candidate_clicked_rec_words (BusInputContext *context,
                                     guint            index,
                                     guint            button,
                                     guint            state)
{
    ...
    if (context->enabled && context->engine) {
        bus_engine_proxy_candidate_clicked_rec_words (context->engine,
                                            index,
                                            button,
                                            state);
    }
...
void
bus_input_context_candidate_clicked_rec_sentences (BusInputContext *context,
                                     guint            index,
                                     guint            button,
                                     guint            state)
{
    ...
    if (context->enabled && context->engine) {
        bus_engine_proxy_candidate_clicked_rec_sentences (context->engine,
                                            index,
                                            button,
                                            state);
    }
...
// David
void
bus_input_context_focused_rec_word (BusInputContext    *context,
                                    IBusText           *sel_rec_word)
{
    g_assert (BUS_IS_INPUT_CONTEXT (context));

    if (context->enabled && context->engine) {
        bus_engine_proxy_focused_rec_word (context->engine,
                                           sel_rec_word);
    }
}

void
bus_input_context_focused_rec_sentence (BusInputContext    *context,
                                        IBusText           *sel_rec_sentence)
{
    g_assert (BUS_IS_INPUT_CONTEXT (context));

    if (context->enabled && context->engine) {
        bus_engine_proxy_focused_rec_sentence (context->engine,
                                               sel_rec_sentence);
    }
}
...
// David
/**
 * bus_input_context_update_focused_area_indicator:
 *
 ...
 */
static void
bus_input_context_update_focused_area_indicator (BusInputContext *context,
                                                 guint            indicator)
{
    g_assert (BUS_IS_INPUT_CONTEXT (context));
...
/**
 * bus_input_context_update_lookup_table_rec_words:
 *
 * Update contents in the lookup table.
 * Send D-Bus signal to update status of client or send glib signal to the panel, depending on capabilities of the client.
 */
static void
bus_input_context_update_lookup_table_rec_words (BusInputContext *context,
                                       IBusLookupTable *table,
                                       gboolean         visible)
{
    g_assert (BUS_IS_INPUT_CONTEXT (context));

    if (context->lookup_table_rec_words) {
        g_object_unref (context->lookup_table_rec_words);
    }

    context->lookup_table_rec_words = (IBusLookupTable *) g_object_ref_sink (table ? table : lookup_table_empty);
    context->lookup_table_visible_rec_words = visible;
...
static void
bus_input_context_update_lookup_table_rec_sentences (BusInputContext *context,
                                       IBusLookupTable *table,
                                       gboolean         visible)
{
    g_assert (BUS_IS_INPUT_CONTEXT (context));

    if (context->lookup_table_rec_sentences ) {
        g_object_unref (context->lookup_table_rec_sentences );
    }

    context->lookup_table_rec_sentences = (IBusLookupTable *) g_object_ref_sink (table ? table : lookup_table_empty);
    context->lookup_table_visible_rec_sentences = visible;
 ...
 */
static void
bus_input_context_show_lookup_table_rec_words (BusInputContext *context)
{
    g_assert (BUS_IS_INPUT_CONTEXT (context));

    if (context->lookup_table_visible_rec_words) {
        return;
    }

    context->lookup_table_visible_rec_words = TRUE;

    if ((context->capabilities & IBUS_CAP_LOOKUP_TABLE) == IBUS_CAP_LOOKUP_TABLE) {
        bus_input_context_emit_signal (context,
                                       "ShowLookupTableRecWords",
                                       NULL,
                                       NULL);
    }
    else {
...
static void
bus_input_context_show_lookup_table_rec_sentences (BusInputContext *context)
{
    g_assert (BUS_IS_INPUT_CONTEXT (context));

    if (context->lookup_table_visible_rec_sentences ) {
        return;
    }

    context->lookup_table_visible_rec_sentences = TRUE;

    if ((context->capabilities & IBUS_CAP_LOOKUP_TABLE) == IBUS_CAP_LOOKUP_TABLE) {
        bus_input_context_emit_signal (context,
                                       "ShowLookupTableRecSentences",
                                       NULL,
                                       NULL);
    }
    else {
 ...
 */
static void
bus_input_context_hide_lookup_table_rec_words (BusInputContext *context)
{
    g_assert (BUS_IS_INPUT_CONTEXT (context));
    ...
    context->lookup_table_visible_rec_words = FALSE;
...
static void
bus_input_context_hide_lookup_table_rec_sentences (BusInputContext *context)
{
    g_assert (BUS_IS_INPUT_CONTEXT (context));
    ...
    context->lookup_table_visible_rec_sentences = FALSE;
...
/**
 * _engine_update_focused_area_indicator_cb
 *
 * A function to be called when "update-focused-area-indicator" glib signal is sent to the engine object.
 */
static void
_engine_update_focused_area_indicator_cb (BusEngineProxy  *engine,
                                          guint            indicator,
                                          BusInputContext *context)
{
    g_assert (BUS_IS_ENGINE_PROXY (engine));
    ...
    bus_input_context_update_focused_area_indicator (context, indicator);
}
...
/**
 * _engine_update_lookup_table_rec_words_cb:
 *
 * A function to be called when "update-lookup-table-rec-words" glib signal is sent to the engine object.
 */
static void
_engine_update_lookup_table_rec_words_cb (BusEngineProxy   *engine,
                                IBusLookupTable  *table,
                                gboolean          visible,
                                BusInputContext  *context)
{
    ...
    bus_input_context_update_lookup_table_rec_words (context, table, visible);
}

static void
_engine_update_lookup_table_rec_sentences_cb (BusEngineProxy   *engine,
                                IBusLookupTable  *table,
                                gboolean          visible,
                                BusInputContext  *context)
{
    ...
    bus_input_context_update_lookup_table_rec_sentences (context, table, visible);
}
...
const static struct {
    ...
    { "update-lookup-table-rec-words",              G_CALLBACK (_engine_update_lookup_table_rec_words_cb) },
    { "update-lookup-table-rec-sentences",          G_CALLBACK (_engine_update_lookup_table_rec_sentences_cb) },
    { "show-lookup-table",                          G_CALLBACK (_engine_show_lookup_table_cb) },
    { "hide-lookup-table",                          G_CALLBACK (_engine_hide_lookup_table_cb) },
    { "show-lookup-table-rec-words",                G_CALLBACK (_engine_show_lookup_table_rec_words_cb) },
    { "hide-lookup-table-rec-words",                G_CALLBACK (_engine_hide_lookup_table_rec_words_cb) },
    { "show-lookup-table-rec-sentences",            G_CALLBACK (_engine_show_lookup_table_rec_sentences_cb) },
    { "hide-lookup-table-rec-sentences",            G_CALLBACK (_engine_hide_lookup_table_rec_sentences_cb) },
    ...
    // David
    { "update-focused-area-indicator",              G_CALLBACK (_engine_update_focused_area_indicator_cb) },
    { NULL, 0 }
};
