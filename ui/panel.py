...
class Panel(ibus.PanelBase):
    ...
    def __init__(self, bus):
        ...
        #outfile.write("panel.py:__init__\n")
        #outfile.close()
        self.__bus = bus
        ...
        #outfile = open("/home/david/David/log.txt","a")
        #outfile.write("panel.py:__init__ MID \n")
        #outfile.close()
        # David
        # Add for rec words
        self.__candidate_panel.connect("page-up-rec-words",
                        lambda widget: self.page_up_rec_words())
        self.__candidate_panel.connect("page-down-rec-words",
                        lambda widget: self.page_down_rec_words())
        self.__candidate_panel.connect("candidate-clicked-rec-words",
                        lambda widget, index, button, state: self.candidate_clicked_rec_words(index, button, state))
        # Add for rec sentences 
        self.__candidate_panel.connect("page-up-rec-sentences",
                        lambda widget: self.page_up_rec_sentences ())
        self.__candidate_panel.connect("page-down-rec-sentences",
                        lambda widget: self.page_down_rec_sentences ())
        self.__candidate_panel.connect("candidate-clicked-rec-sentences",
                        lambda widget, index, button, state: self.candidate_clicked_rec_sentences (index, button, state))

        # David 
        # connect to the signal from the candidatepanel, and then emit DBus signal

        # self.__candidate_panel.connect("focused-rec-word",self.__focused_rec_word_cb)
        self.__candidate_panel.connect("focused-rec-word",
                        lambda widget, sel_rec_word: self.focused_rec_word(sel_rec_word))
        self.__candidate_panel.connect("focused-rec-sentence",
                        lambda widget, sel_rec_sentence: self.focused_rec_sentence(sel_rec_sentence))
        ...
        #outfile = open("/home/david/David/log.txt","a")
        #outfile.write("panel.py:__init__ finish\n")
        #outfile.close()
    ...
    def update_lookup_table_rec_words(self, lookup_table, visible):
        self.__candidate_panel.update_lookup_table_rec_words(lookup_table, visible)

    def show_lookup_table_rec_words(self):
        self.__candidate_panel.show_lookup_table_rec_words()

    def hide_lookup_table_rec_words(self):
        self.__candidate_panel.hide_lookup_table_rec_words()

    def update_lookup_table_rec_sentences(self, lookup_table, visible):
        self.__candidate_panel.update_lookup_table_rec_sentences(lookup_table, visible)

    def show_lookup_table_rec_sentences(self):
        self.__candidate_panel.show_lookup_table_rec_sentences()

    def hide_lookup_table_rec_sentences(self):
        self.__candidate_panel.hide_lookup_table_rec_sentences()
    ...
    # David
    # this method the corresponding method of the base class
    def update_focused_area_indicator(self, indicator):
        self.__candidate_panel.update_focused_area_indicator(indicator)
    ...
    # message preprocess
    def __focused_rec_word_cb(self, candidatepanel,recword):
        #outfile = open("/home/david/David/log.txt","a")
        #outfile.write("panel.py:before process glib signal["+str(recword)+"]\n")
        #outfile.close()
        if recword == None or recword == "":
            return
        self.focused_rec_word(ibus.Text(recword))
        #outfile = open("/home/david/David/log.txt","a")
        #outfile.write("panel.py:after process glib signal["+str(recword)+"]\n")
        #outfile.close()
