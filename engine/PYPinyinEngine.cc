...
void
PinyinEngine::candidateClickedRecWords (guint index, guint button, guint state)
{
    m_editors[m_input_mode]->candidateClickedRecWords (index, button, state);
}

void
PinyinEngine::candidateClickedRecSentences (guint index, guint button, guint state)
{
    m_editors[m_input_mode]->candidateClickedRecSentences (index, button, state);
}

// David
// These are signals from the UI
// Need write some corresponding method in the PYPhoneticEditor.cc
// Words
void
PinyinEngine::pageUpRecWords (void)
{
    //m_editors[m_input_mode]->pageUp ();
    m_editors[m_input_mode]->pageUpRecWords  ();
}

void
PinyinEngine::pageDownRecWords  (void)
{
    m_editors[m_input_mode]->pageDownRecWords  ();
}

void
PinyinEngine::cursorUpRecWords  (void)
{
    m_editors[m_input_mode]->cursorUpRecWords  ();
}

void
PinyinEngine::cursorDownRecWords  (void)
{
    m_editors[m_input_mode]->cursorDownRecWords  ();
}

// Sentences 
void
PinyinEngine::pageUpRecSentences (void)
{
    m_editors[m_input_mode]->pageUpRecSentences  ();
}

void
PinyinEngine::pageDownRecSentences (void)
{
    m_editors[m_input_mode]->pageDownRecSentences  ();
}

void
PinyinEngine::cursorUpRecSentences (void)
{
    m_editors[m_input_mode]->cursorUpRecSentences  ();
}

void
PinyinEngine::cursorDownRecSentences (void)
{
    m_editors[m_input_mode]->cursorDownRecSentences  ();
}

// David
void
PinyinEngine::focusedRecWord (IBusText *sel_rec_word)
{
    if (m_input_mode != MODE_INIT)
        return;
...
void
PinyinEngine::focusedRecSentence (IBusText *sel_rec_sentence)
{
    if (m_input_mode != MODE_INIT)
        return;
...
{ 
    // I think I need add the rec word and sentence commit here
...
PinyinEngine::connectEditorSignals (EditorPtr editor)
{
    ...
    editor->signalUpdateLookupTableRecWords ().connect (
        std::bind (&PinyinEngine::updateLookupTableRecWords, this, _1, _2));
    editor->signalUpdateLookupTableFastRecWords ().connect (
        std::bind (&PinyinEngine::updateLookupTableFastRecWords, this, _1, _2));
    editor->signalShowLookupTableRecWords ().connect (
        std::bind (&PinyinEngine::showLookupTableRecWords, this));
    editor->signalHideLookupTableRecWords ().connect (
        std::bind (&PinyinEngine::hideLookupTableRecWords, this));

    editor->signalUpdateLookupTableRecSentences ().connect (
        std::bind (&PinyinEngine::updateLookupTableRecSentences , this, _1, _2));
    editor->signalUpdateLookupTableFastRecSentences ().connect (
        std::bind (&PinyinEngine::updateLookupTableFastRecSentences , this, _1, _2));
    editor->signalShowLookupTableRecSentences ().connect (
        std::bind (&PinyinEngine::showLookupTableRecSentences , this));
    editor->signalHideLookupTableRecSentences ().connect (
        std::bind (&PinyinEngine::hideLookupTableRecSentences , this));

    // David
    // bind the signal with the corresponding method
    editor->signalUpdateFocusedAreaIndicator ().connect (
        std::bind (&PinyinEngine::updateFocusedAreaIndicator, this, _1));
