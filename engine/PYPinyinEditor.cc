...
PinyinEditor::processNumber (guint keyval, guint keycode, guint modifiers)
{
    ...
    if (modifiers == 0) {
        //m_log.logInfo ("[%s][%s]indicator[%d]selectCandidateInpage[%d]\n", __FILE__, __FUNCTION__, m_focused_area_indicator, i);
        selectCandidateInPage (i);
    } else if ((modifiers & ~IBUS_LOCK_MASK) == IBUS_CONTROL_MASK) {
        //m_log.logInfo ("[%s][%s]indicator[%d]resetCandidateInpage[%d]\n", __FILE__, __FUNCTION__, m_focused_area_indicator, i);
...
PinyinEditor::commit ()
{
    ...
    //m_log.logInfo ("[%s][%s]m_phrase_editor.selectedString is [%s]\n", __FILE__, __FUNCTION__, m_phrase_editor.selectedString ().c_str ());
    ...
    //m_log.logInfo ("[%s][%s]m_buffer is [%s]\n", __FILE__, __FUNCTION__, m_buffer.c_str ());
...
PinyinEditor::updatePreeditText ()
{
    ...
    // David add
    m_preedit_text = preedit_text.text();
