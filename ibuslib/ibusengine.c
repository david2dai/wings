...
static void      ibus_engine_candidate_clicked
                                             (IBusEngine         *engine,
                                              ...
                                              guint               state);
// David
static void      ibus_engine_focused_rec_word 
                                              (IBusEngine     *engine,
                                               IBusText       *sel_rec_word);
static void      ibus_engine_focused_rec_sentence 
                                                  (IBusEngine     *engine,
                                                   IBusText       *sel_rec_sentence);

static void      ibus_engine_page_up_rec_words                  (IBusEngine         *engine);
static void      ibus_engine_page_down_rec_words                (IBusEngine         *engine);
static void      ibus_engine_cursor_up_rec_words                (IBusEngine         *engine);
static void      ibus_engine_cursor_down_rec_words              (IBusEngine         *engine);
static void      ibus_engine_candidate_clicked_rec_words         
                                                                (IBusEngine         *engine,
                                                                 guint               index,
                                                                 guint               button,
                                                                 guint               state);
static void      ibus_engine_page_up_rec_sentences              (IBusEngine         *engine);
static void      ibus_engine_page_down_rec_sentences            (IBusEngine         *engine);
static void      ibus_engine_cursor_up_rec_sentences            (IBusEngine         *engine);
static void      ibus_engine_cursor_down_rec_sentences          (IBusEngine         *engine);
static void      ibus_engine_candidate_clicked_rec_sentences           
                                                                (IBusEngine         *engine,
                                                                 guint               index,
                                                                 guint               button,
                                                                 guint               state);
...
static const gchar introspection_xml[] =
    ...
    "    </method>"
    // David
    "    <method name='CandidateClickedRecWords'>"
    "      <arg direction='in'  type='u' name='index' />"
    "      <arg direction='in'  type='u' name='button' />"
    "      <arg direction='in'  type='u' name='state' />"
    "    </method>"
     "    <method name='CandidateClickedRecSentences'>"
    "      <arg direction='in'  type='u' name='index' />"
    "      <arg direction='in'  type='u' name='button' />"
    "      <arg direction='in'  type='u' name='state' />"
    "    </method>"
    ...
    // David
    "    <method name='PageUpRecWords' />"
    "    <method name='PageDownRecWords' />"
    "    <method name='CursorUpRecWords' />"
    "    <method name='CursorDownRecWords' />"
    "    <method name='PageUpRecSentences' />"
    "    <method name='PageDownRecSentences' />"
    "    <method name='CursorUpRecSentences' />"
    "    <method name='CursorDownRecSentences' />"
    //  
    ...
    "    </method>"
    // David
    "    <method name='FocusedRecWord'>"
    "      <arg direction='in'  type='v' name='sel_rec_word' />"
    "    </method>"
    "    <method name='FocusedRecSentence'>"
    "      <arg direction='in'  type='v' name='sel_rec_sentence' />"
    "    </method>"
    /* FIXME signals */
    ...
    "    </signal>"
    // David
    // the signal without signal was not defined here
    // search: non param DBus signals
    "    <signal name='UpdateFocusedAreaIndicator'>"
    "      <arg type='u' name='indicator' />"
    "    </signal>"
    "    <signal name='UpdateLookupTableRecWords'>"
    "      <arg type='v' name='table' />"
    "      <arg type='b' name='visible' />"
    "    </signal>"
    "    <signal name='UpdateLookupTableRecSentences'>"
    "      <arg type='v' name='table' />"
    "      <arg type='b' name='visible' />"
    "    </signal>"
    "  </interface>"
...
ibus_engine_class_init (IBusEngineClass *class)
{
    ...
    // David 
    // Some of the following will be rewrite in the sub class of the class (like PYEngine in the engine)
    ...
    // David 
    class->focused_rec_word     = ibus_engine_focused_rec_word;
    class->focused_rec_sentence = ibus_engine_focused_rec_sentence;

    class->page_up_rec_words                = ibus_engine_page_up_rec_words;
    class->page_down_rec_words              = ibus_engine_page_down_rec_words;
    class->cursor_up_rec_words              = ibus_engine_cursor_up_rec_words;
    class->cursor_down_rec_words            = ibus_engine_cursor_down_rec_words;
    class->candidate_clicked_rec_words      = ibus_engine_candidate_clicked_rec_words;

    class->page_up_rec_sentences            = ibus_engine_page_up_rec_sentences;
    class->page_down_rec_sentences          = ibus_engine_page_down_rec_sentences;
    class->cursor_up_rec_sentences          = ibus_engine_cursor_up_rec_sentences;
    class->cursor_down_rec_sentences        = ibus_engine_cursor_down_rec_sentences;
    class->candidate_clicked_rec_sentences  = ibus_engine_candidate_clicked_rec_sentences;
    ...
    // David
    engine_signals[PAGE_UP_REC_WORDS] =
        g_signal_new (I_("page-up-rec-words"),
            G_TYPE_FROM_CLASS (gobject_class),
            G_SIGNAL_RUN_LAST,
            G_STRUCT_OFFSET (IBusEngineClass, page_up_rec_words),
            NULL, NULL,
            _ibus_marshal_VOID__VOID,
            G_TYPE_NONE,
            0);

    engine_signals[PAGE_DOWN_REC_WORDS] =
        g_signal_new (I_("page-down-rec-words"),
            G_TYPE_FROM_CLASS (gobject_class),
            G_SIGNAL_RUN_LAST,
            G_STRUCT_OFFSET (IBusEngineClass, page_down_rec_words),
            NULL, NULL,
            _ibus_marshal_VOID__VOID,
            G_TYPE_NONE,
            0);

    engine_signals[CURSOR_UP_REC_WORDS] =
        g_signal_new (I_("cursor-up-rec-words"),
            G_TYPE_FROM_CLASS (gobject_class),
            G_SIGNAL_RUN_LAST,
            G_STRUCT_OFFSET (IBusEngineClass, cursor_up_rec_words),
            NULL, NULL,
            _ibus_marshal_VOID__VOID,
            G_TYPE_NONE,
            0);

    engine_signals[CURSOR_DOWN_REC_WORDS] =
        g_signal_new (I_("cursor-down-rec-words"),
            G_TYPE_FROM_CLASS (gobject_class),
            G_SIGNAL_RUN_LAST,
            G_STRUCT_OFFSET (IBusEngineClass, cursor_down_rec_words),
            NULL, NULL,
            _ibus_marshal_VOID__VOID,
            G_TYPE_NONE,
            0);
    ...
    // Sentences
    engine_signals[PAGE_UP_REC_SENTENCES] =
        g_signal_new (I_("page-up-rec-sentences"),
            G_TYPE_FROM_CLASS (gobject_class),
            G_SIGNAL_RUN_LAST,
            G_STRUCT_OFFSET (IBusEngineClass, page_up_rec_sentences),
            NULL, NULL,
            _ibus_marshal_VOID__VOID,
            G_TYPE_NONE,
            0);

    engine_signals[PAGE_DOWN_REC_SENTENCES] =
        g_signal_new (I_("page-down-rec-sentences"),
            G_TYPE_FROM_CLASS (gobject_class),
            G_SIGNAL_RUN_LAST,
            G_STRUCT_OFFSET (IBusEngineClass, page_down_rec_sentences),
            NULL, NULL,
            _ibus_marshal_VOID__VOID,
            G_TYPE_NONE,
            0);

    engine_signals[CURSOR_UP_REC_SENTENCES] =
        g_signal_new (I_("cursor-up-rec-sentences"),
            G_TYPE_FROM_CLASS (gobject_class),
            G_SIGNAL_RUN_LAST,
            G_STRUCT_OFFSET (IBusEngineClass, cursor_up_rec_sentences),
            NULL, NULL,
            _ibus_marshal_VOID__VOID,
            G_TYPE_NONE,
            0);

    engine_signals[CURSOR_DOWN_REC_SENTENCES] =
        g_signal_new (I_("cursor-down-rec-sentences"),
            G_TYPE_FROM_CLASS (gobject_class),
            G_SIGNAL_RUN_LAST,
            G_STRUCT_OFFSET (IBusEngineClass, cursor_down_rec_sentences),
            NULL, NULL,
            _ibus_marshal_VOID__VOID,
            G_TYPE_NONE,
            0);
    ...
    // David
    engine_signals[CANDIDATE_CLICKED_REC_WORDS] =
        g_signal_new (I_("candidate-clicked-rec-words"),
            G_TYPE_FROM_CLASS (gobject_class),
            G_SIGNAL_RUN_LAST,
            G_STRUCT_OFFSET (IBusEngineClass, candidate_clicked_rec_words),
            NULL, NULL,
            _ibus_marshal_VOID__UINT_UINT_UINT,
            G_TYPE_NONE,
            3,
    ...
    engine_signals[CANDIDATE_CLICKED_REC_SENTENCES] =
        g_signal_new (I_("candidate-clicked-rec-sentences"),
            G_TYPE_FROM_CLASS (gobject_class),
            G_SIGNAL_RUN_LAST,
            G_STRUCT_OFFSET (IBusEngineClass, candidate_clicked_rec_sentences),
            NULL, NULL,
            _ibus_marshal_VOID__UINT_UINT_UINT,
            G_TYPE_NONE,
            3,
    ...
    // David
    engine_signals[FOCUSED_REC_WORD] =
        g_signal_new (I_("focused-rec-word"),
            G_TYPE_FROM_CLASS (gobject_class),
            G_SIGNAL_RUN_LAST,
            G_STRUCT_OFFSET (IBusEngineClass, focused_rec_word),
            NULL, NULL,
    ...
    // David
    engine_signals[FOCUSED_REC_SENTENCE] =
        g_signal_new (I_("focused-rec-sentence"),
            G_TYPE_FROM_CLASS (gobject_class),
            G_SIGNAL_RUN_LAST,
            G_STRUCT_OFFSET (IBusEngineClass, focused_rec_sentence),
            NULL, NULL,
            _ibus_marshal_VOID__OBJECT,
            G_TYPE_NONE,
            1,
                                 ...
                                 GDBusMethodInvocation *invocation)
{
    ...
    if (g_strcmp0 (method_name, "CandidateClickedRecWords") == 0) {
        guint index, button, state;
        g_variant_get (parameters, "(uuu)", &index, &button, &state);
        g_signal_emit (engine,
                       engine_signals[CANDIDATE_CLICKED_REC_WORDS],
    ...
    if (g_strcmp0 (method_name, "CandidateClickedRecSentences") == 0) {
        guint index, button, state;
        g_variant_get (parameters, "(uuu)", &index, &button, &state);
        g_signal_emit (engine,
                       engine_signals[CANDIDATE_CLICKED_REC_SENTENCES],
    ...
    // David
    if (g_strcmp0 (method_name, "FocusedRecWord") == 0) {
        GVariant *variant = NULL;
        IBusText *text;
        ...
        g_signal_emit (engine, engine_signals[FOCUSED_REC_WORD],
                       0,
                       text);
    ...
    // David
    if (g_strcmp0 (method_name, "FocusedRecSentence") == 0) {
        GVariant *variant = NULL;
        IBusText *text;
        ...
        g_signal_emit (engine, engine_signals[FOCUSED_REC_SENTENCE],
                       0,
                       text);
...
// David
static void
ibus_engine_page_up_rec_words                  (IBusEngine         *engine)
{
}

static void
ibus_engine_page_down_rec_words                (IBusEngine         *engine)
{
}

static void
ibus_engine_cursor_up_rec_words                (IBusEngine         *engine)
{
}

static void
ibus_engine_cursor_down_rec_words              (IBusEngine         *engine)
{
}

static void
ibus_engine_candidate_clicked_rec_words         
                                                                (IBusEngine         *engine,
                                                                 guint               index,
                                                                 guint               button,
                                                                 guint               state)
...
static void
ibus_engine_page_up_rec_sentences              (IBusEngine         *engine)
{
}

static void
ibus_engine_page_down_rec_sentences            (IBusEngine         *engine)
{
}

static void
ibus_engine_cursor_up_rec_sentences            (IBusEngine         *engine)
{
}


static void
ibus_engine_cursor_down_rec_sentences          (IBusEngine         *engine)
{
}


static void
ibus_engine_candidate_clicked_rec_sentences           
                                                                (IBusEngine         *engine,
                                                                 guint               index,
                                                                 guint               button,
                                                                 guint               state)
...
static void        
ibus_engine_focused_rec_word (IBusEngine     *engine,
                              IBusText       *sel_rec_word)
{
...
static void        
ibus_engine_focused_rec_sentence (IBusEngine     *engine,
                                  IBusText       *sel_rec_sentence)
{
...
// David 
// for updating the panel focused area
void ibus_engine_update_focused_area_indicator  (IBusEngine         *engine,
                                                 guint               indicator)
{
    /*
    FILE *outfile; 
    ...
    char buffer[256] = {0};
    sprintf(buffer,"ibusengine.c:ibus_engine_update_focused_area_indicator\n");
...
void
ibus_engine_update_lookup_table_rec_words (IBusEngine        *engine,
                                 IBusLookupTable   *table,
                                 gboolean           visible)
{
    g_return_if_fail (IBUS_IS_ENGINE (engine));
...
void
ibus_engine_update_lookup_table_fast_rec_words (IBusEngine        *engine,
                                      IBusLookupTable   *table,
                                      gboolean           visible)
{
    ...
    if (table->candidates->len < table->page_size << 2) {
        ibus_engine_update_lookup_table_rec_words (engine, table, visible);
        return;
    }
    ...
    ibus_engine_update_lookup_table_rec_words (engine, new_table, visible);
...
// Sentences
void
ibus_engine_update_lookup_table_rec_sentences (IBusEngine        *engine,
                                 IBusLookupTable   *table,
                                 gboolean           visible)
{
    g_return_if_fail (IBUS_IS_ENGINE (engine));
...
void
ibus_engine_update_lookup_table_fast_rec_sentences (IBusEngine        *engine,
                                      IBusLookupTable   *table,
                                      gboolean           visible)
{
    g_return_if_fail (IBUS_IS_ENGINE (engine));
    ...
    if (table->candidates->len < table->page_size << 2) {
        ibus_engine_update_lookup_table_rec_sentences (engine, table, visible);
        return;
    }
    ...
    ibus_engine_update_lookup_table_rec_sentences (engine, new_table, visible);
...
// David
// non param DBus signals
DEFINE_FUNC (show_lookup_table_rec_words, ShowLookupTableRecWords)
DEFINE_FUNC (hide_lookup_table_rec_words, HideLookupTableRecWords)
DEFINE_FUNC (show_lookup_table_rec_sentences, ShowLookupTableRecSentences)
DEFINE_FUNC (hide_lookup_table_rec_sentences, HideLookupTableRecSentences)
